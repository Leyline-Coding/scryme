...
async def sell_list(session: AsyncSession, currency: str = "usd") -> SellList:
    ...
    cards = [
        SellCard(
            ...
            unit=unit_price(prices, "normal", currency),
        )
        for sid, name, set_code, set_name, rarity, cn, prices, qty in rows
    ]
