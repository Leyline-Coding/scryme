...
| `SCRYME_DEFAULT_CURRENCY` | `usd` | Default display currency for current values: `usd` / `eur` (native Scryfall prices) or `gbp` / `cad` / `aud` / `jpy` (converted from USD via a daily FX rate); visitors override in Settings |
